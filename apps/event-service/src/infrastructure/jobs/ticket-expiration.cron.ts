...
const releaseExpiredTicketsUseCase = new ReleaseExpiredTicketsUseCase(ticketRepository);
...
export function startTicketExpirationJob() {
  ...
  cron.schedule('* * * * *', async () => {
    try {
      ...
      console.error('[Cron Worker Error] Failed to release expired tickets:', error);
    }
  });
